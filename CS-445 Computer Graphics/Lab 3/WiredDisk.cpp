...
// ---------------------------------------------------------------------- default constructor
WiredDisk::WiredDisk(int numSections)
{
    ...
    {
        tri(index, vec4(0,0,0,1), vertices[j], vertices[j+1] , vertex_colors[j]);
    }//for loop
}
...
void
WiredDisk::tri(int &index, vec4 center, vec4 v1, vec4 v2, vec4 col)
{
    colors[index] = col;
    points[index] = center;
