...
Cone::Cone(int numSections)
{
    ...
    {
        tri(index, vec4(0,height,0,1), vertices[j], vertices[j+1] , vertex_colors[j], face_normals[0]);
    }//for loop
}
...
void
Cone::tri(int &index, vec4 center, vec4 v1, vec4 v2, vec4 col, vec4 norm)
{
    colors[index] = col;
    points[index] = center;
    normals[index] = norm;
